/**
 * WorkerS Pro Editor - Node.js VPS Server Version
 * 移植说明:
 * 1. 移除 Cloudflare 特定对象 (env, ctx)
 * 2. 使用 SQLite (better-sqlite3) 替代 D1
 * 3. 使用 Express 托管服务
 */
...
// --- 初始化 ---
const app = express();
const upload = multer(); // 用于解析 multipart/form-data
  ...
  try {
    ...
    try {
      ...
      if (text.includes("Could not route") || text.includes("object identifier is invalid")) {
        ...
        data = { success: false, errors: [{ message: "API Token 无效或权限不足" }] };
      } else {
...
const dbOps = {
  getAllTokens: () => {
    return db.prepare('SELECT * FROM user_tokens ORDER BY created ASC').all() || [];
  },
  saveToken: (t) => {
    const stmt = db.prepare('INSERT OR REPLACE INTO user_tokens (id, token, created, expiry, boundAccountId, boundApiToken) VALUES (?, ?, ?, ?, ?, ?)');
    stmt.run(t.id, t.token, t.created, t.expiry, t.boundAccountId, t.boundApiToken);
  },
  deleteToken: (id) => {
    db.prepare('DELETE FROM user_tokens WHERE id = ?').run(id);
  }
};

const generateComplexToken = () => {
...
app.post('/', upload.none(), async (req, res) => {
  try {
    ...
    let apiToken = formData.apiToken;
    const loginToken = formData.loginToken;
    ...
    if (loginToken) {
      ...
      if (!matchedToken) {
        return jsonRes(res, { success: false, errors: [{ message: "无效的 Token (未找到)" }] });
      }
      
      if (matchedToken.expiry !== -1 && Date.now() > matchedToken.expiry) {
        return jsonRes(res, { success: false, errors: [{ message: "Token 已过期" }] });
      }
    ...
    if (['listTokens', 'createToken', 'deleteToken'].includes(action)) {
      if (!hasAuth) return jsonRes(res, { success: false, errors: [{ message: "权限验证失败" }] });

      if (action === 'listTokens') {
        let tokens = dbOps.getAllTokens();
        const now = Date.now();
        // 隐藏真实 API Key
        ...
        return jsonRes(res, { success: true, result: safeTokens });
      }

      if (action === 'createToken') {
        const expiryInput = parseInt(formData.expiryDays || '-1');
        if (expiryInput > 365) return jsonRes(res, { success: false, errors: [{ message: "自定义天数不能超过 365 天" }] });
        ...
        dbOps.saveToken(newToken);
        return jsonRes(res, { success: true, result: newToken });
      }

      if (action === 'deleteToken') {
        dbOps.deleteToken(formData.tokenId);
    ...
    if (!accountId || !apiToken) {
      if (action === 'verifyLogin') return jsonRes(res, { success: false, errors: [{ message: "Account ID 或 Token 不能为空" }] });
      return jsonRes(res, { success: false, errors: [{ message: "缺少认证信息" }] });
    }
    ...
    if (action === 'verifyLogin') {
      const apiRes = await safeFetch(`${baseUrl}/workers/scripts?per_page=1`, { headers: authHeader });
      if (apiRes.ok) {
        return jsonRes(res, { success: true, role: isRootAdmin ? 'root' : 'token' });
      } else {
        const errorMsg = apiRes.data.errors && apiRes.data.errors[0] ? apiRes.data.errors[0].message : "验证失败，凭证无效";
        return jsonRes(res, { success: false, errors: [{ message: errorMsg }] });
      }
    }
    ...
    if (action === "createResource") {
       ...
       if (!name) return jsonRes(res, { success: false, errors: [{ message: "名称不能为空" }] });
       ...
       if (type === 'kv_namespace') {
         ...
         return jsonRes(res, { success: false, errors: [{ message: "暂不支持创建此类型资源" }] });
       }
       ...
       return jsonRes(res, apiRes.data);
    }
    ...
    if (action === "listResources") {
      ...
      let url = "";
      const urlMap = {
        'kv_namespace': `${baseUrl}/storage/kv/namespaces`,
        'd1': `${baseUrl}/d1/database`,
        'r2_bucket': `${baseUrl}/r2/buckets`,
        'service': `${baseUrl}/workers/scripts`,
        'queue': `${baseUrl}/queues`,
        'durable_object_namespace': `${baseUrl}/workers/durable_objects/namespaces`,
        'vectorize': `${baseUrl}/vectorize/indexes`,
        'hyperdrive': `${baseUrl}/hyperdrive/configs`,
        'dispatch_namespace': `${baseUrl}/workers/dispatch/namespaces`,
        'analytics_engine': `${baseUrl}/analytics_engine/datasets`
      };
      url = urlMap[type];

      if (!url) return jsonRes(res, { success: true, result: [] });
      const apiRes = await safeFetch(url, { headers: authHeader });
      return jsonRes(res, apiRes.data);
    }

    // 获取脚本列表
    if (action === "listScripts") {
      const apiRes = await safeFetch(`${baseUrl}/workers/scripts`, { headers: authHeader });
      return jsonRes(res, apiRes.data);
    }

    // 拉取脚本代码
    if (action === "fetch") {
      const scriptName = formData.scriptName;
      if (!scriptName) return jsonRes(res, { success: false, errors: [{ message: "脚本名称为空" }] });

      const contentRes = await fetch(`${baseUrl}/workers/scripts/${scriptName}`, { headers: authHeader });
      let code = "";
      ...
      return jsonRes(res, { success: true, code, bindings });
    }
    ...
    if (action === "deploy") {
      ...
      const incomingBindings = formData.bindings;
      ...
      return jsonRes(res, deployRes.data);
    }

  } catch (err) {
    console.error("System Error:", err);
    return jsonRes(res, { success: false, errors: [{ message: `系统错误: ${err.message}` }] });
  }
});
    ...
    <div class="custom-content-wrapper" id="main-interface">
      ...
      <div class="header-row">
        ...
        <div class="header-actions">
          ...
          <a href="https://github.com/Kevin-YST-Du/Cloudflare-Workers" target="_blank" class="action-icon-btn" title="访问 GitHub 仓库">
