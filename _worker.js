/**
 * Worker Pro Editor - 纯净修正版
 * 修改项：
 * 1. Account ID 更改为明文显示 (type="text")
 * 2. 标题 Worker Pro Editor 不加粗 (font-normal)
 * 3. 报错信息支持鼠标选中复制
 * 4. 保持代码格式化，不进行压缩
 */
...
<body class="light">
  <div class="custom-content-wrapper">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
      ...
      <input id="token" type="password" placeholder="API Token" class="p-4 rounded-xl text-lg w-full bg-slate-50 border border-slate-200 shadow-inner outline-none">
    </div>
    ...
    <div class="modal-card">
      <div class="text-center">
        ...
        <div class="flex gap-3">
          <button onclick="closeModal('deploy-modal')" class="flex-1 py-4 rounded-xl font-bold bg-slate-100 text-slate-600 hover:bg-slate-200 transition">取消</button>
          <button onclick="executeDeploy()" class="flex-1 py-4 rounded-xl font-bold bg-red-600 text-white shadow-lg shadow-red-500/30 transition">立即同步</button>
        </div>
      </div>
    </div>
  </div>
    ...
    async function doAction(action, extra = {}) {
      ...
      try {
        ...
        if(res.success) {
          ...
          showToast(res.errors[0].message, true); 
        }
      } catch(e) { 
        showToast("⚠️ 请求异常", true); 
      }
    }
